...
class User {
  ...
  findAll() {
    return new Promise((resolve) => {
      let results = _.filter(sampleData[this.tableName], { userId: this.id, ...this.conditions });
  ...
  findOne() {
    return new Promise((resolve) => {
      let result = _.find(sampleData[this.tableName], { userId: this.id, ...this.conditions });
      if (result) {
